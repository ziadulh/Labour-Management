...
class ReportController extends Controller
{
	...
	public function buildingCostReport(){
		...
		$getAllBuildingsReport=DB::select(DB::raw("SELECT subQuery2.total_cost,subQuery2.amount as paid_amount, subQuery2.builId as id, subQuery2.buildName,  subQuery2.total_salary,SUM(salary__based_logs.salary) as total_salary_cost
			FROM
			(
			SELECT subQuery.total_cost,subQuery.amount, subQuery.builId, subQuery.buildName, SUM (salary__based__employees.salary) as total_salary
			FROM 
			(SELECT SUM(rate.rate*rate.total_hazira) as amount, SUM(rate.total_cost) as total_cost, rate.builId as builId, rate.buildName as buildName
			FROM 
			...
			GROUP BY buildings.id,labours.attendance_rate) rate
			LEFT JOIN buildings
			ON buildings.id = rate.builId
			GROUP BY rate.builId,rate.buildName) subQuery
			LEFT JOIN salary__based__employees
			ON salary__based__employees.building_id = subQuery.builId


			GROUP BY salary__based__employees.building_id,subQuery.total_cost,subQuery.amount, subQuery.builId, subQuery.buildName
			) subQuery2
			LEFT JOIN salary__based_logs
			ON salary__based_logs.building_id = subQuery2.builId
			GROUP BY subQuery2.total_salary,subQuery2.total_cost,subQuery2.amount, subQuery2.builId, subQuery2.buildName, salary__based_logs.building_id "));
	...
	public function groupCostReport(){
		...
		$group = DB::select(DB::raw("	SELECT  SUM(subquery.rate * subquery.total_hazira) as total_amount, SUM(subquery.total_cost) as total_paid, groups.name as groupname, subquery.groupid as groupid
			FROM
			( 
			SELECT SUM(salary_logs.food_rate_paid) as total_cost, salary_logs.group_id as groupId,labours.attendance_rate as rate, salary_logs.labour_id,SUM (salary_logs.attendence_number) as total_hazira
			FROM salary_logs 
			LEFT JOIN labours
			ON labours.id = salary_logs.labour_id

			GROUP BY salary_logs.labour_id,labours.attendance_rate,salary_logs.group_id 
			) subquery
			LEFT JOIN groups
			ON subquery.groupid = groups.id

			GROUP BY subquery.groupid, groups.name
			")); 

		return view('report.groupCost',compact(['group']));
	}
